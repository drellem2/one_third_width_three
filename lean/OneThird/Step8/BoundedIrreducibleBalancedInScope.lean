...
`bounded_irreducible_balanced_inScope` (`mg-d783`)
...
* G1' (`mg-b487`, merged) — `enumPosetsFor_iter_balanced`: the per-mask
  ...
  `hasAdjacentIncomp`).
* G3a (`mg-609a`, merged) — `findSymmetricPair_isSome_imp_symmetric_aux`:
  Bool→Prop lift for the fast-path symmetric witness extraction.
  Carries `successorMasks` bit-correctness as an explicit hypothesis
  (G3a-followup `mg-792a` is open).
...
* G3c (`mg-1906`, merged partial) — band-major positional consequences
  of `cross_band_lt_upward` and `enumNOf (bandSizes L) = Fintype.card α`.
  The full `enumPredAtMaskOf = predMaskOf` construction-equivalence and
  the `enumFreeUVOf = freeUVOf` alignment are still open
  (G3c-followup `mg-7268`); they are carried as explicit hypotheses
  here per pc-b53a's recommended Path 1.
...
The signature carries five caller-side obligations:
...
2. `h_construction_eq` — the per-mask construction-equivalence
   (G3c-followup, OPEN).
3. `h_freeUV_eq` — the free-UV alignment between the enumerator and
   `L`'s band-major free pairs (G3c-followup, OPEN).
4. `h_succ` — bit-correctness of `successorMasks` on `predMaskOf L`
   (G3a-followup, OPEN).
5. `h_certificate` — `Case3Enum.enumPosetsFor L.w (bandSizes L) = true`,
...
Each of these obligations becomes trivial once the corresponding
followup lands; until then, this signature lets downstream consumers
(F5 strong-induction driver, A5-G3e composition) build against a
fully-discharged theorem body.
...
The five caller-side hypotheses cover the still-open dependencies
(G3a/G3c followups) and the F5a Bool certificate's
`L`-specific consequence. -/
theorem bounded_irreducible_balanced_inScope
    ...
    (h_construction_eq :
      Case3Enum.enumPredAtMaskOf L.w (bandSizes L) (maskOf L) =
        predMaskOf L)
    (h_freeUV_eq :
      Case3Enum.enumFreeUVOf L.w (bandSizes L) = freeUVOf L)
    (h_succ : ∀ u v : ℕ, u < Fintype.card α → v < Fintype.card α →
      Nat.testBit ((Case3Enum.successorMasks (predMaskOf L)
            (Fintype.card α)).getD u 0) v =
        Case3Enum.testBit' ((predMaskOf L).getD v 0) u)
    (h_certificate :
  ...
  -- G3a discharge: lift `findSymmetricPair = some _` to a `Symmetric`
  -- witness, modulo the open `successorMasks` bit-correctness.
  have h_sym_lift :
    ...
    fun h =>
      Case3Enum.findSymmetricPair_isSome_imp_symmetric_aux
        (predMaskOf L) (Fintype.card α) h_succ h
