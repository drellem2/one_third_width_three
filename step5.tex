...
\GAP{The bad-set bound $B_0(n) = O(n^3)$ used in step~(I2) above is
imported from Theorem~\ref{thm:interface}~(iv) of Step~1. The mg-5fdd
audit (\texttt{docs/onethird-math-finished-audit.md}) flags a possible
\emph{circular dependency}: Step~1's $O(n\cdot t)$ bad-set estimate
appears to be upgraded to the form consumed here only by assuming a
positive lower bound $|\fib_{x,y}| \ge c\,n^2$ on the good-fiber mass
--- which is itself a richness statement of the kind
Lemma~\ref{lem:fiber-mass} is meant to \emph{establish}. If so, Step~1
and Step~5 each rely on the other for the positive-$|\LP|$-fraction
fact, and neither proves it unconditionally. This finding was raised
by a skeptical re-audit and is flagged here pending a dedicated
verification of Theorem~\ref{thm:interface}~(iv).}
