...
\paragraph{The in-tree residual obligations.}\label{sec:roadmap-residual}
A companion Lean formalisation (directory \texttt{lean/OneThird/};
see \texttt{lean/README.md} and \texttt{lean/AXIOMS.md}) tracks the
in-tree status of every load-bearing result of this paper. At
present, that formalisation reduces the headline 1/3--2/3 theorem to
two precisely-typed residual obligations, one of which is closed by
external enumeration and one of which remains conditional on
Steps~1--7:
\begin{enumerate}\setlength{\itemsep}{2pt}
  \item \emph{Small-$n$ slice ($|P| \le 10$):} the paper-level proof
  is unconditional via the strong induction in
  Lemma~\ref{lem:layered-balanced} (Section~\ref{sec:g4-balanced-pair}).
  The in-tree Lean dispatch verifies the singleton-band sub-slice
  ...
  \texttt{enum\_cap5\_certificate.json}). Coverage of the remaining
  width-$3$ non-chain $|P| \le 10$ configurations (e.g.\ reducible
  posets like $A \oplus B$ with $A,B$ width-$3$ antichains, whose
  minimum singleton-band bandwidth exceeds $4$) requires either an
  extended enumeration or a Lean port of
  Lemma~\ref{lem:layered-balanced} that drops the singleton-band
  restriction (companion work item \texttt{mg-2970}).
  \item \emph{Large-$n$ slice ($|P| \ge 11$):} the in-tree discharge
  is conditional on a faithful Lean delivery of the Step~7
  bandwidth bound --- Proposition~\ref{prop:72} of
  \texttt{step7.tex}, pinned to interaction width $w = w_0(\gamma)
  \le 4$ (the constant aligned with the Lean certificate
  \texttt{InCase3Scope.w\_mem}). The
  paper-level proof of Prop.~\ref{prop:72} (existence of a layered
  decomposition of width $w = O_T(1)$) is complete; only the
  faithful Lean re-derivation of the constant $w_0(\gamma) \le 4$
  remains open in tree.
\end{enumerate}
Read as a paper-level statement, the headline is unconditional
(modulo the arithmetic-richness hypothesis stated in Step~8). Read
as an in-tree Lean claim, the small-$n$ slice is unconditional and
the large-$n$ slice is conditional on Steps~1--7 of the present
paper. The faithful in-tree delivery of those steps is the present
formalisation's principal remaining work.
