...
This paper proves the $1/3$--$2/3$ conjecture at width~$3$.
...
quantitative chain of constants developed across Steps~1--7.
...
\begin{theorem}[Width-$3$ $1/3$--$2/3$]
\label{thm:conclusion-main}
Let $P$ be a finite poset with $w(P)\le 3$ that is not a total order.
Then $P$ has a \emph{balanced pair}: two incomparable elements
$x,y\in P$ with
\[
...
$n_0(\gamma,T)$ polynomial in $1/\gamma$ and $T$. Below $n_0$ a
direct finite-enumeration argument (Remark~\ref{rem:small-n})
supplies the base case; above it the BK-expansion contradiction
...
closes both branches. The argument drives the excess $\gamma$
across the cascade down to $0$, so the bound of
