...
posets.  This paper develops a programme toward the conjecture for
posets of width~$3$.
...
The argument proceeds in eight steps: (1)~a local interface/grid
...
\medskip\noindent\textbf{Status of the argument.} The argument is
presented here in full, but it is \emph{not} at present a complete
proof. An internal audit (companion file
\texttt{docs/onethird-math-finished-audit.md}) has identified several
load-bearing steps whose proofs are currently incomplete --- notably
the area-vs.-perimeter scaling in the Step~2 per-fiber stability
estimate, the two-interface incompatibility bound of Step~4, and the
deep irreducible case of the Step~8 layered balanced-pair lemma. These
are marked in the text with \textbf{[GAP:]} annotations. The main
theorem below should accordingly be read as conditional on those gaps
being closed, in addition to the arithmetic-richness
Hypothesis~\ref{hyp:arith} stated in Step~8.
\end{abstract}
...
This paper develops an argument toward the $1/3$--$2/3$ conjecture at
width~$3$. The argument is conditional on an arithmetic richness
hypothesis (Hypothesis~A below; see Hypothesis~\ref{hyp:arith} in
Step~8 for the precise form) that pins the intermediate regime
$\gamma < 1/3 - \delta_{\mathrm{KL}} \approx 0.057$,
$n > n_0(\gamma, T)$; and, as an internal audit has found (see the
\emph{Status of the argument} note in the abstract and
Section~\ref{sec:roadmap-residual}), it is at present also conditional
on several load-bearing steps whose proofs are incomplete.

\begin{theorem}[Main theorem, conditional on Hypothesis~A and on the
audit-identified gaps]
\label{thm:main}
Assume the arithmetic richness Hypothesis~A of Step~8
(Hypothesis~\ref{hyp:arith}), and assume the load-bearing steps marked
\textbf{[GAP:]} in the text (catalogued in
Section~\ref{sec:roadmap-residual}) admit valid proofs. Let $P$ be a
finite poset of width at most~$3$ that is not a chain. Then $P$ admits
a pair $(x,y)$ of incomparable elements with
\[
  \tfrac{1}{3} \;\le\; \Pr[x<y] \;\le\; \tfrac{2}{3}.
\]
Equivalently, $\delta(P)\ge 1/3$ for every non-chain poset $P$ of
width~$\le 3$, under these hypotheses.
\end{theorem}
...
regime via the BK-expansion cascade. The audit-identified gaps lie
within Steps~2, 4 and~8 and within the Step~1/Step~5 richness
interface; they are catalogued, with the corresponding
\textbf{[GAP:]} annotations, in Section~\ref{sec:roadmap-residual}.
...
\paragraph{Audit status: the load-bearing gaps.}\label{sec:roadmap-residual}
An internal audit of this paper (work item \texttt{mg-5fdd};
companion file \texttt{docs/onethird-math-finished-audit.md})
identified several load-bearing steps whose proofs, as written, are
incomplete. Each is marked at its place in the text with a
\textbf{[GAP:]} annotation. They are, in logical order:
\begin{enumerate}\setlength{\itemsep}{2pt}
  \item \emph{Step~2 --- per-fiber staircase stability
  (Proposition~\ref{prop:per-fiber}).} The proof feeds an
  \emph{area}-scaled per-fiber boundary bound ($\le\eta|\fiber{x,y}|$,
  $\eta$ a fixed threshold) into the \emph{perimeter}-scaled stability
  Lemma~\ref{lem:weak-grid} (which needs $\bd_D A\le\varepsilon t$
  with $\varepsilon$ small), via $\varepsilon:=\eta\,t$. For rich
  fibers with side length $t=\Theta(n)$ this makes $\varepsilon$
  diverge, so the exported ``$o(1)$-close to a staircase'' conclusion
  is not established. This propagates into Steps~3, 4, 6 and~7.
  \item \emph{Step~4 --- two-interface incompatibility at $|R|$-scale.}
  Lemma~\ref{lem:rect-stable-area} is vacuous (its own
  Remark~\ref{rem:G3-vacuity} concedes this); the re-routing through a
  $\min(m,n)$-scale lemma falls a factor $t$ short of the $|R|$-scale
  expansion bound that Theorem~\ref{thm:step4} claims.
  \item \emph{Step~1 / Step~5 --- richness interface.} The bad-set
  bound of Theorem~\ref{thm:interface}~(iv) and the fiber-mass lower
  bound of Lemma~\ref{lem:fiber-mass} appear to depend circularly on
  one another for the positive-$|\mathcal L(P)|$-fraction fact;
  flagged pending verification.
  \item \emph{Step~8 G4 --- deep irreducible layered balanced pair.}
  Lemma~\ref{lem:window-localization} is invalid for interaction
  width $w\ge1$, so Case~C1 of Lemma~\ref{lem:layered-balanced} is
  unsound; the alternative deep-case route
  Lemma~\ref{lem:layered-reduction} has a wrong window-perturbation
  estimate \eqref{eq:window-pert}. The deep \emph{irreducible} layered
  width-$3$ case --- the case into which a minimal counterexample
  falls --- is therefore unproven by either route. The statement is
  strongly supported by exhaustive computation (over $10^6$ posets, no
  counterexample) but is not proved.
\end{enumerate}
In addition, the headline is conditional on Hypothesis~A
(Hypothesis~\ref{hyp:arith}), and the small-$n$ base case
(Lemma~\ref{lem:small-n}, Remark~\ref{rem:small-n}) relies for the
small-$\gamma$ regime on a finite enumeration that is effective in
principle but has been carried out only for $n$ up to roughly $11$.

A companion Lean formalisation (directory \texttt{lean/OneThird/};
see \texttt{lean/README.md} and \texttt{lean/AXIOMS.md}) tracks the
in-tree status of the load-bearing results. It is sorry-free but
rests on five disclosed project axioms --- \texttt{brightwell\_sharp\_centred}
(the external Brightwell/Kahn--Saks bound),
\texttt{lem\_layered\_balanced\_irreducible\_base} (the Step~8 G4 deep
irreducible residual of item~4 above), \texttt{stepsOneToSevenCascade}
(all of Steps~1--7, items~1--3 above), \texttt{width3\_smallN\_hasBalancedPair}
(the base case), and \texttt{nonChain\_compl\_of\_no\_balancedPair}
(the perturbation-step non-chain fact). Of these, only the first is a
pure formalisation gap (an un-ported but complete external result);
the others shadow the genuine mathematical gaps catalogued above. See
\texttt{docs/onethird-math-finished-audit.md} for the per-axiom
analysis.
