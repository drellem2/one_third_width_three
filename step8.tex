...
\begin{remark}[In-tree headline restatement; Lean
formalisation status]\label{rem:headline-lean-restatement}
The companion Lean formalisation
(\texttt{lean/OneThird/MainTheorem.lean}, \texttt{width3\_one\_third\_two\_thirds})
exposes Theorem~\ref{thm:main-step8} as a disjoint union of two
slices that the paper's single conditional statement bundles:
\begin{enumerate}[nosep, label=(\roman*)]
  \item \emph{Small-$n$ unconditional slice $(|P| \le 10)$.} For
  every width-$3$ finite poset on at most $10$ elements that is not
  a chain, $P$ has a balanced pair. \emph{Paper-level:}
  unconditional via the strong induction in
  Lemma~\ref{lem:layered-balanced} below
  (Section~\ref{sec:g4-balanced-pair}); does not depend on
  Steps~1--7 or on Hypothesis~\ref{hyp:arith}. \emph{In-tree Lean
  dispatch:} verifies the singleton-band cap-$w \le 4$ sub-slice
  ($P$ admitting a width-$\le 4$ singleton-band layered
  decomposition) by exhaustive Python enumeration
  (\texttt{lean/scripts/enum\_cap5.py}; certificate
  \texttt{enum\_cap5\_certificate.json}; see
  Remark~\ref{rem:residual-base}). Coverage of the remaining
  configurations (e.g.\ reducible $|P| = 6$ posets like $A \oplus B$
  with $A,B$ width-$3$ antichains, whose minimum singleton-band
  bandwidth equals $5$) requires either an extended enumeration or
  a Lean port of Lemma~\ref{lem:layered-balanced} that drops the
  singleton-band restriction (companion work item
  \texttt{mg-2970}).
  \item \emph{Large-$n$ Steps-1--7-conditional slice $(|P| \ge 11)$.}
  For every width-$3$ non-chain finite poset on at least $11$
  elements, $P$ has a balanced pair, conditional on
  Prop.~\ref{prop:72} of \texttt{step7.tex} delivering a layered
  width-3 decomposition of bandwidth $w_0(\gamma) \le 4$ (see
  Remark~\ref{rem:w0-constant} of \texttt{step7.tex}).
\end{enumerate}
The Lean dispatch shape uses Lemma~\ref{lem:layered-balanced} (G4)
uniformly at every depth $K \ge 1$ via an F3 strong induction inside
\texttt{Case3WitnessProof.lean}; the depth-dichotomy of
Section~\ref{sec:layered-reduction} (G3 for $K \ge K_0$, G4 for
$K < K_0$) is a paper-side organisation that the in-tree dispatch
does not consume on the headline path. The G3 lemma
(Lemma~\ref{lem:layered-reduction}) is nonetheless formalised
end-to-end in Lean (\texttt{Step8/LayeredReduction.lean},
\texttt{lem\_cut}, \texttt{lem\_layered\_reduction}) and is
preserved as documentation of the paper's intended mechanism; see
the disclaimer at the head of Section~\ref{sec:layered-reduction}.
\end{remark}
...
force. The companion Lean formalisation does \emph{not} consume
Hypothesis~\ref{hyp:arith}: the in-tree headline
\texttt{width3\_one\_third\_two\_thirds} is stated against the
Steps~1--7 interface (paper-axiomatised at
\texttt{lean/OneThird/Step7/Assembly.lean}, \texttt{LayeredWidth3})
and dispatches via the bandwidth field directly; the cascade
closure inequality~\eqref{eq:G2} appears in the paper only, at the
Steps~1--7 assembly level, and is not re-derived in Lean. The
constants of this section are therefore paper-side bookkeeping for
the Steps~1--7 assembly; the in-tree formalisation's surface area
of conditionality is limited to the
\hyperref[rem:headline-lean-restatement]{large-$n$
Steps-1--7-conditional slice} of
Remark~\ref{rem:headline-lean-restatement}.
\end{remark}
...
\texttt{case3\_certificate}). The two proofs prove the same theorem
under the same hypothesis; the in-tree formalisation differs only in
dispatch shape.
\end{remark}
...
(in its size-minimal one-shot form) directly produces a balanced
pair in $P$ — using the cut + bulk/window lift internally and
contradicting that $P$ is a $\gamma$-counter\-example, without
recursing on $\gamma$ or descending to a smaller counterexample at
the outer assembly level. For $K < K_0$, Lemma~\ref{lem:layered-balanced}
...
Proposition~\ref{prop:step7}(iii) without any depth-gap and without
any compounding $\gamma$-decay; the residual enumeration is the
sole remaining mathematical obligation above the $w = 0$ floor. The
G3/G4 dichotomy of the paper is a logical organisation; the
companion Lean formalisation absorbs both regimes into G4 via the F3
strong induction of \texttt{Case3WitnessProof.lean}
(Remarks~\ref{rem:layered-balanced-lean} and
\ref{rem:g3-paper-only}).
\end{remark}
...
For this residual regime we appeal to a bounded finite enumeration:
window localization (Lemma~\ref{lem:window-localization}) caps the
size of $Q^\star$ at $|Q^\star| \le 3(3w+1)$ and the depth at
$K^\star \le 2w+2$. With $w \in \{1,2,3,4\}$ from the bandwidth
constant $w_0(\gamma)$ of Remark~\ref{rem:w0-constant} of
\texttt{step7.tex}, the residual parameter range is finite and has
been \emph{exhaustively verified} by the enumeration described in
Remark~\ref{rem:enumeration} above: under the cap-$1$ singleton-band
profile + cap-$5$ bandwidth bound $w \le 4$, the effective
parameter domain is $|Q| = K \le 2w + 2 \le 10$, yielding $15$
$(K, w)$ tuples covering roughly $1.72\times 10^6$ configurations
across $K \in \{2, \ldots, 10\}$ and $w \in \{0, \ldots, 4\}$
subject to $K \le 2w + 2$. The verification proceeds in two layers:
the parameter cells $(K, w) \in \{(3,1), (3,2), (3,3), (3,4),
(4,1)\}$ are closed in tree by the F5a Bool certificate
(Lean: \texttt{lean/OneThird/Step8/Case3Enum/},
\texttt{case3\_certificate}, via \texttt{native\_decide}); the
residual $15$ cells with $K \in \{4, \ldots, 10\}$ and $w \in
\{2, 3, 4\}$ are verified by Python enumeration
(\texttt{lean/scripts/enum\_cap5.py} for $K \le 9$;
\texttt{enum\_cap5\_K10.py} for $K = 10$, $w = 4$; companion work
item \texttt{mg-4d7b}). The accompanying certificate
\texttt{enum\_cap5\_certificate.json} records the per-cell
configuration count and reports no counterexample; a counterexample
would halt the enumeration and be flagged.
\end{remark}
