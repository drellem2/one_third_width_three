...
\begin{enumerate}[nosep, label=(\roman*)]
 ...
 \item (Prop.~7.3) A width-$3$ poset $P$ that is layered in the
 sense of (ii) or of Proposition~\ref{prop:step5}(C) has a balanced
 pair: in the shallow regime ($K < K_0$), unconditionally
 (Lemma~\ref{lem:layered-balanced}); in the deep regime ($K \ge
 K_0$), under the additional hypothesis that $P$ is size-minimal
 among width-$3$ $\gamma$-counterexamples
 (Lemma~\ref{lem:layered-reduction}, in its size-minimal one-shot
 form). The size-minimality assumption is harmless in the
 main-theorem cascade (Theorem~\ref{thm:main-step8}), where the
 ambient $\gamma$-counterexample is taken size-minimal at the
 outset.
\end{enumerate}
\end{proposition}
...
\begin{remark}[Companion Lean formalisation: status]\label{rem:headline-lean-restatement}
The companion Lean formalisation
(\texttt{lean/OneThird/MainTheorem.lean}, \texttt{width3\_one\_third\_two\_thirds})
exposes Theorem~\ref{thm:main-step8} as a \texttt{sorry}-free
development resting on \emph{five disclosed project axioms}
(\texttt{lean/AXIOMS.md}). The headline Lean theorem is a proof by
contradiction --- a minimal $\gamma$-counter\-example is fed through
Theorem~E, the Steps~1--7 cascade, the Step~7--8 bridge, the Step~8
G4 lemma and the exceptional-set perturbation lift --- and it takes
Hypothesis~A (\texttt{HypothesisArithmetic}) as an explicit
hypothesis. The five axioms are: \texttt{brightwell\_sharp\_centred}
(the external Brightwell/Kahn--Saks bound \eqref{eq:sharp-centred});
\texttt{stepsOneToSevenCascade} (all of Steps~1--7);
\texttt{lem\_layered\_balanced\_irreducible\_base} (the Step~8 G4
deep irreducible residual --- the open gap discussed after the proof
of Lemma~\ref{lem:layered-balanced});
\texttt{width3\_smallN\_hasBalancedPair} (the base case
Lemma~\ref{lem:small-n}); and
\texttt{nonChain\_compl\_of\_no\_balancedPair} (the perturbation-step
non-chain fact). An internal audit (work item \texttt{mg-5fdd},
\texttt{docs/onethird-math-finished-audit.md}) found that, of these,
only \texttt{brightwell\_sharp\_centred} is a pure formalisation gap
(a complete external result not yet ported); the remaining four
shadow the genuine, in-places-open mathematical gaps marked
\textbf{[GAP:]} in this paper (see
Section~\ref{sec:roadmap-residual}). The G3 reduction lemma
(Lemma~\ref{lem:layered-reduction}) is also formalised in Lean
(\texttt{Step8/LayeredReduction.lean}); it inherits the gap recorded
in the \textbf{[GAP:]} note after its proof.
\end{remark}
...
\begin{itemize}[nosep]
 ...
 Lemma~\ref{lem:layered-reduction} (G3) to $P$ in its size-minimal
 form, treating $P$ as exactly layered via
 Remark~\ref{rem:layered-exact-assumption} (the perturbation budget
 of~\eqref{eq:exc-perturb}/\eqref{eq:n0-main} is absorbed into the
 lemma's $K_0$). Because $P$ is the size-minimal width-$3$
 $\gamma$-counter\-example of the cascade, the lemma's
 size-minimality hypothesis is satisfied; G3 directly yields a
 balanced pair $(x,y)$ of $P$ with $p_{xy}(P) \in [1/3, 2/3]$,
 contradicting that $P$ is a $\gamma$-counter\-example. (No
 recursive descent on $\gamma$: the lemma's $K_0$ depends one-sidedly
 on $\gamma$ via the window-perturbation bound alone.)
 \item If $K < K_0$, apply Lemma~\ref{lem:layered-balanced} (G4)
 directly. First, $P|_{X \setminus X^{\mathrm{exc}}}$ is not a chain:
 if it were, then $P$ would consist of a chain on
 $n - |X^{\mathrm{exc}}|$ elements together with the
 $|X^{\mathrm{exc}}| \le C_{\mathrm{exc}}(T) = O_T(1)$ exceptional
 elements; by Lemma~\ref{lem:indec-incompairs} the indecomposable
 poset $P$ has $\ge n/2$ incomparable pairs, each of which (since the
 complement is a chain) involves an exceptional element, so by
 pigeonhole some exceptional $z$ is incomparable to at least
 $n / (2\,C_{\mathrm{exc}}(T))$ chain elements; these form a
 contiguous range $[c_a, c_b]$ of the chain (the chain elements
 comparable to $z$ split into an up-set and a down-set), and a floater
 incomparable to a contiguous chain-range of length $\ell$ has
 $\Pr[z <_L c_j] \in [1/3, 2/3]$ for some mid-range $c_j$ once
 $\ell \ge 2$ (the floater is inserted into one of $\ell + 1$ gaps
 with monotone, $1/(\ell+1)$-spaced cumulative probabilities), giving
 a balanced pair of $P$ directly --- already a contradiction. So
 $P|_{X \setminus X^{\mathrm{exc}}}$ is not a chain, and
 Lemma~\ref{lem:layered-balanced} produces a balanced pair $(x,y)$;
 the same perturbation bound~\eqref{eq:exc-perturb} transfers this to
 $P$ via $p_{xy}(P) \in [1/3 - \gamma/2, 2/3 + \gamma/2]$,
 contradicting that $P$ is a $\gamma$-counter\-example.
\end{itemize}
\GAP{The non-chain argument for $P|_{X \setminus X^{\mathrm{exc}}}$
just given is correct in outline but is a \emph{sketch}: the
``floater incomparable to a long contiguous chain-range yields a
balanced pair'' step uses, for the marginal $\Pr_P[z <_L c_j]$ over
the \emph{full} poset $P$, that the cumulative probabilities along the
range are monotone with bounded ($O(1/\ell)$) increments. The earlier
draft justified ``$P|_{X \setminus X^{\mathrm{exc}}}$ is not a chain''
by the false one-liner ``else $P$ were a chain'' (deleting $O_T(1)$
elements from a non-chain can yield a chain). A fully rigorous
self-contained lemma for the floater step --- the Lean axiom
\texttt{nonChain\_compl\_of\_no\_balancedPair} --- remains to be
written; the mathematics is standard. See
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).}
...
\smallskip\noindent\emph{Direct derivation of the difference.} We
now establish~\eqref{eq:one-elem-perturb} by a direct computation on
the weighted means $F_A$, $|A|$, $\bar f$, bypassing the two-factor
split of~\eqref{eq:diff-final}. By~\eqref{eq:pxy-fibers}, using
$N = \bar f N'$,
\begin{equation}\label{eq:diff-identity}
  p_{xy}(Q) - p_{xy}(Q-z)
  \;=\; \frac{F_A}{N} - \frac{|A|}{N'}
  \;=\; \frac{F_A - \bar f |A|}{N}
  \;=\; \frac{1}{N}\sum_{L' \in A}\!\bigl(f(L') - \bar f\bigr).
\end{equation}
This is the identity underlying~\eqref{eq:diff-final}: rewriting both
sides of~\eqref{eq:diff-final} over a common denominator gives
$p_{xy}(Q) - \Pr(A \mid \bar B) = \frac{m}{m - \bar f}\,
(p_{xy}(Q) - p_{xy}(Q-z))$, so the ``second factor'' bound is
algebraically equivalent to a bound on the centred sum
$\sum_{L' \in A}(f(L') - \bar f)$ via the same
quantity~\eqref{eq:diff-identity}.

\smallskip\noindent\emph{$1$-Lipschitz structure of $f$, $S$, $P$.}
Observe first that both $S$ and $P$ are \emph{$1$-Lipschitz} on the
adjacent-transposition Cayley graph of $\mathcal L(Q-z)$: if
$L', L'' \in \mathcal L(Q-z)$ differ by transposing a single
incomparable pair $(u, v)$ at adjacent ranks $r, r+1$, then at most
one of $u, v$ lies in $\operatorname{pred}(z) \cup \operatorname{succ}(z)$
(if one were in $\operatorname{pred}(z)$ and the other in
$\operatorname{succ}(z)$, transitivity through~$z$ would make them
comparable, contradicting the validity of the swap). Hence the swap
affects at most one of the sets
$\{\operatorname{pos}_{L'}(u) : u \in \operatorname{pred}(z)\}$ and
$\{\operatorname{pos}_{L'}(v) : v \in \operatorname{succ}(z)\}$, and
by at most one position; so
\begin{equation}\label{eq:fS-fP-lipschitz}
  |S(L') - S(L'')| \le 1,\qquad |P(L') - P(L'')| \le 1,
\end{equation}
with at most one of the two being strict, and consequently
$|f(L') - f(L'')| \le 1$.

The antisymmetric representation
$\sum_{L' \in A}(f(L') - \bar f) = \frac{1}{N'}\sum_{L' \in A,\,L'' \in A^c}(f(L') - f(L''))$
(valid because $\sum_{L' \in \mathcal L(Q-z)}(f(L') - \bar f) = 0$)
together with the global bound $|f(L') - f(L'')| \le m - 1$ already
yields the weak bound
\begin{equation}\label{eq:weak-centred}
  \Bigl|\sum_{L' \in A}(f(L') - \bar f)\Bigr|
  \;\le\; \frac{(m-1)\,|A|\,|A^c|}{N'}.
\end{equation}
However~\eqref{eq:weak-centred} translates via~\eqref{eq:diff-identity}
to $|\Delta| \le (m-1)/(4\bar f)$, weaker than the target $2/m$ except
when $\bar f$ is comparable to $m$. The sharp bound requires
exploiting the monotonicity of $S, P, \mathbf{1}_A$ with respect to
the lattice order on $\mathcal L(Q-z)$ via the Ahlswede--Daykin
inequality.

\smallskip\noindent\emph{Sharp bound via pred/succ separation and the
Ahlswede--Daykin inequality.} We now establish
\begin{equation}\label{eq:sharp-centred}
  \Bigl|\sum_{L' \in A}(f(L') - \bar f)\Bigr| \;\le\; \frac{2N}{m},
\end{equation}
which, dividing by~$N$ and combining with~\eqref{eq:diff-identity},
gives $|p_{xy}(Q) - p_{xy}(Q-z)| \le 2/m$, i.e.,
\eqref{eq:one-elem-perturb}.

\emph{Step~1 (reduction to a covariance on a product lattice).}
Let $\mu := \operatorname{Unif}(\mathcal L(Q-z))$ and let
$\nu := \mu \otimes \operatorname{Unif}\{1, \ldots, m\}$ be the
product measure on the product space
$\mathcal X := \mathcal L(Q-z) \times \{1, \ldots, m\}$. In the joint
coupling, $B := \{(L', J) : P(L') < J \le S(L')\}$ is the event that
$J$ is a valid rank for inserting $z$ into~$L'$. Then
$\Pr_\nu(B) = \bar f/m$, and conditional on $B$ the induced extension
$L \in \mathcal L(Q)$ obtained by inserting $z$ at rank $J$ is uniform
on $\mathcal L(Q)$. Let $A := \{(L', J) : x <_{L'} y\}$, which depends
only on~$L'$; then $\Pr_\nu(A) = p_{xy}(Q-z)$ and
$\Pr_\nu(A \mid B) = p_{xy}(Q)$. By Bayes,
\begin{equation}\label{eq:delta-as-cov}
  \Delta := p_{xy}(Q) - p_{xy}(Q-z)
  \;=\; \frac{\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_B)}{\Pr_\nu(B)}
  \;=\; \frac{m}{\bar f}\,\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_B).
\end{equation}
Decompose the complement $B^c = B_- \sqcup B_+$ into the two disjoint
"too-early" and "too-late" sub-events
\begin{equation}\label{eq:Bbar-decomp}
  B_- := \{(L', J) : J \le P(L')\},
  \qquad
  B_+ := \{(L', J) : J > S(L')\},
\end{equation}
with
$\Pr_\nu(B_-) = \mathbb E_\mu[P]/m$ and
$\Pr_\nu(B_+) = \mathbb E_\mu[m - S]/m$, so
$\Pr_\nu(B_-) + \Pr_\nu(B_+) = 1 - \bar f/m$.
From $\mathbf{1}_B = 1 - \mathbf{1}_{B_-} - \mathbf{1}_{B_+}$,
\begin{equation}\label{eq:cov-split}
  \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_B)
  \;=\; -\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_-})
  \;-\; \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_+}).
\end{equation}
The pred/succ separation is apparent: $B_-$ depends on $L'$ only
through $P$ (a function of $\operatorname{pred}(z)$-positions), while
$B_+$ depends on $L'$ only through $S$ (a function of
$\operatorname{succ}(z)$-positions); the two sub-events interact with
$A$ through \emph{disjoint} element subsets of $Q-z$.

\emph{Step~2 (distributive-lattice structure on $\mathcal X$).} The
set $\mathcal L(Q-z)$ carries a finite distributive-lattice structure
$(\mathcal L(Q-z), \le_{\mathcal D})$: fix a reference linear
extension $\tau \in \mathcal L(Q-z)$ satisfying
\begin{itemize}[nosep]
 \item all elements of $\operatorname{pred}(z)$ precede all elements of
 $\operatorname{succ}(z)$ in $\tau$ (possible since any
 $u \in \operatorname{pred}(z)$ and $v \in \operatorname{succ}(z)$ satisfy
 $u <_Q z <_Q v$, forcing $u <_\tau v$ in any extension);
 \item $x <_\tau y$ (WLOG; if $(x,y)$ is comparable in $Q-z$, $A$ has
 probability $0$ or~$1$ and~\eqref{eq:sharp-centred} is trivial).
\end{itemize}
Declare $L' \le_{\mathcal D} L''$ iff the set of
$\tau$-inversions of $L'$ is contained in that of $L''$; this makes
$\mathcal L(Q-z)$ a distributive lattice whose bottom is $\tau$ and
whose atoms are adjacent transpositions of incomparable pairs
(see~\cite[Prop.~3.5.1]{Brightwell1999}). Endow $\{1, \ldots, m\}$
with its natural chain order, and form the product distributive
lattice $\mathcal X$ with componentwise order. The product measure
$\nu$ is uniform on $\mathcal X$, hence log-supermodular, so by the
Ahlswede--Daykin inequality~\cite{AhlswedeDaykin1978} (equivalently
FKG~\cite{FKG1971} for a uniform measure on a finite distributive
lattice), for any two monotone functions
$g, h : \mathcal X \to \mathbb R$ of the \emph{same} type (both
nondecreasing or both nonincreasing),
\begin{equation}\label{eq:FKG-lattice}
  \operatorname{Cov}_\nu(g, h) \;\ge\; 0;
\end{equation}
if $g$ is nondecreasing and $h$ nonincreasing (or vice versa), the
inequality reverses.

\emph{Step~3 (monotonicity of $A$, $B_-$, $B_+$).} Under the chosen
$\tau$:
\begin{enumerate}[label=(\alph*),nosep]
 \item $A = \{x <_{L'} y\}$ is an \emph{order ideal} in $\mathcal L(Q-z)$
 (i.e., $\mathbf{1}_A$ is nonincreasing on $\le_{\mathcal D}$): moving
 up in $\le_{\mathcal D}$ only adds $\tau$-inversions, and $(x,y)$ with
 $x <_\tau y$ is $\tau$-non-inverted in $A$ but inverted in $A^c$. On
 the product $\mathcal X$, $\mathbf{1}_A$ depends only on the first
 coordinate and remains nonincreasing.

 \item $L' \mapsto P(L')$ is \emph{nondecreasing} on $\le_{\mathcal D}$.
 Indeed, $\tau$ places $\operatorname{pred}(z)$ first; the bottom
 $L' = \tau$ has pred$(z)$-elements at the smallest possible rank
 positions, so $P(\tau)$ is minimal among $\mathcal L(Q-z)$. Any
 single-swap step up in $\le_{\mathcal D}$ transposes an incomparable
 pair to create a new $\tau$-inversion; by the pred/succ separation,
 such a swap either (i) transposes two pred$(z)$-elements or two
 non-pred elements (unchanged max), (ii) transposes a
 pred$(z)$-element with a non-pred element strictly later in $\tau$,
 moving the pred-element later (nondecreasing $P$). So $P$ is
 nondecreasing. On $\mathcal X$, extend $P$ by ignoring the
 $J$-coordinate; it remains nondecreasing.

 \item Symmetrically, $L' \mapsto S(L')$ is \emph{nonincreasing} on
 $\le_{\mathcal D}$: the top of $\mathcal D$ places
 $\operatorname{succ}(z)$ at the smallest ranks, and moving up in
 $\le_{\mathcal D}$ can only push succ-elements earlier, decreasing
 the min.

 \item Define $\mathbf{1}_{B_-}(L', J) = \mathbf{1}[J \le P(L')]$: on
 the product lattice (with natural order in the $J$-coordinate),
 $\mathbf{1}_{B_-}$ is nondecreasing in~$L'$ (by~(b)) and
 nonincreasing in~$J$. To apply~\eqref{eq:FKG-lattice} uniformly,
 work with the order $J \le^* J'$ iff $J \ge J'$ (\emph{reverse
 order} on the chain); this is still a distributive lattice, $\nu$
 remains uniform hence log-supermodular, and
 $\mathbf{1}_{B_-}$ is now nondecreasing in both coordinates.

 \item $\mathbf{1}_{B_+}(L', J) = \mathbf{1}[J > S(L')]$ is
 nondecreasing in $L'$ (since $S$ is nonincreasing by~(c), so
 $\{J > S\}$ becomes more likely as $L'$ goes up) and nondecreasing
 in $J$, hence jointly nondecreasing on $\mathcal X$ with the natural
 product order.
\end{enumerate}

\emph{Step~4 (sign of the two covariances via
Ahlswede--Daykin).} In the product lattice with the natural
$J$-order, $\mathbf{1}_A$ is nonincreasing (by~(a)) and
$\mathbf{1}_{B_+}$ is nondecreasing (by~(e)), so
by~\eqref{eq:FKG-lattice}
$\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_+}) \le 0$.
In the product lattice with the reversed $J$-order,
$\mathbf{1}_A$ is still nonincreasing (it does not depend on $J$)
and $\mathbf{1}_{B_-}$ is nondecreasing (by~(d)); the uniform
measure $\nu$ is invariant under reversing the chain order, so
$\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_-}) \le 0$.
Both covariances are $\le 0$, hence
\begin{equation}\label{eq:AD-signs}
  \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_-}) \le 0,
  \qquad
  \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_+}) \le 0,
\end{equation}
and combining with~\eqref{eq:cov-split},
$\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_B) \ge 0$, which
by~\eqref{eq:delta-as-cov} gives $\Delta \ge 0$ under our choice
$x <_\tau y$. (This is Brightwell's FKG-based sign
theorem~\cite[\S4]{Brightwell1999}; the opposite choice gives
$\Delta \le 0$ symmetrically.)

\emph{Step~5 (collapse to a $\mu$-covariance and quantitative bound).}
Since the $J$-marginal expectations evaluate in closed form
\[
  \mathbb E_\nu[\mathbf{1}_{B_-}\mid L'] \;=\; P(L')/m,
  \qquad
  \mathbb E_\nu[\mathbf{1}_{B_+}\mid L'] \;=\; (m-S(L'))/m,
\]
and $\mathbf{1}_A$ depends only on $L'$, the product-space covariances
reduce to $\mu$-covariances:
\begin{equation}\label{eq:cov-as-mu}
  \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_-})
   \;=\; \frac{1}{m}\,\operatorname{Cov}_\mu(\mathbf{1}_A, P),
  \qquad
  \operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_{B_+})
   \;=\; -\frac{1}{m}\,\operatorname{Cov}_\mu(\mathbf{1}_A, S).
\end{equation}
Substituting into~\eqref{eq:cov-split},
$\operatorname{Cov}_\nu(\mathbf{1}_A, \mathbf{1}_B)
 = \frac{1}{m}(\operatorname{Cov}_\mu(\mathbf{1}_A, S)
               - \operatorname{Cov}_\mu(\mathbf{1}_A, P))
 = \frac{1}{m}\operatorname{Cov}_\mu(\mathbf{1}_A, f)$,
recovering~\eqref{eq:delta-as-cov} in the form
$\Delta = \operatorname{Cov}_\mu(\mathbf{1}_A, f)/\bar f$
of~\eqref{eq:diff-identity}. The sign
constraints~\eqref{eq:AD-signs} translate, via~\eqref{eq:cov-as-mu},
into
\begin{equation}\label{eq:mu-signs}
  \operatorname{Cov}_\mu(\mathbf{1}_A, P) \le 0,
  \qquad
  \operatorname{Cov}_\mu(\mathbf{1}_A, S) \ge 0,
\end{equation}
so the two $\mu$-covariances contribute with \emph{opposite} signs
to $\operatorname{Cov}_\mu(\mathbf{1}_A, f)
 = \operatorname{Cov}_\mu(\mathbf{1}_A, S) - \operatorname{Cov}_\mu(\mathbf{1}_A, P) \ge 0$,
and in particular
\begin{equation}\label{eq:cov-abs}
  |\operatorname{Cov}_\mu(\mathbf{1}_A, f)|
  \;=\; \operatorname{Cov}_\mu(\mathbf{1}_A, S)
       - \operatorname{Cov}_\mu(\mathbf{1}_A, P)
  \;=\; |\operatorname{Cov}_\mu(\mathbf{1}_A, S)|
       + |\operatorname{Cov}_\mu(\mathbf{1}_A, P)|.
\end{equation}
It suffices, therefore, to bound each summand on the right
of~\eqref{eq:cov-abs} by $\bar f/m$.

\emph{Per-term bound (Kahn--Saks; Brightwell).} The quantitative
bound
\begin{equation}\label{eq:per-term-KS}
  |\operatorname{Cov}_\mu(\mathbf{1}_A, S)| \le \bar f/m,
  \qquad
  |\operatorname{Cov}_\mu(\mathbf{1}_A, P)| \le \bar f/m,
\end{equation}
is the single-element-perturbation lemma of
Kahn--Saks~\cite[Lemma~2.2]{KahnSaks1984} and
Brightwell~\cite[Theorem~4.1]{Brightwell1999}, applied to the two
disjoint element subsets $\operatorname{succ}(z)$ and
$\operatorname{pred}(z)$ respectively. The proof is a direct
combinatorial computation: starting from the sign
constraints~\eqref{eq:mu-signs} (which localize the covariance sign
via AD), the $1$-Lipschitz
bounds~\eqref{eq:fS-fP-lipschitz} (which control the fluctuation
scale of $S$ and $P$ under single swaps), and the pred/succ
disjointness clause of~\eqref{eq:fS-fP-lipschitz} (which prevents
simultaneous saturation of the two bounds), one constructs a
canonical exchange involution
$\sigma : \mathcal L(Q-z) \to \mathcal L(Q-z)$ with
$\sigma(A) = A^c$ such that $|S(L') - S(\sigma L')| \le 1$ and
$|P(L') - P(\sigma L')| \le 1$ for every $L'$, and such that the
insertion-position averaging $\mathbb E_\nu[\cdot \mid L']$ produces
the factor $1/m$. Combining~\eqref{eq:per-term-KS}
with~\eqref{eq:cov-abs},
\[
  |\operatorname{Cov}_\mu(\mathbf{1}_A, f)| \;\le\; \frac{2\bar f}{m},
\]
and by $N = \bar f N'$ and the identity
$\sum_{L' \in A}(f(L') - \bar f) = N' \operatorname{Cov}_\mu(\mathbf{1}_A, f)$,
\[
  \Bigl|\sum_{L' \in A}(f(L') - \bar f)\Bigr|
  \;=\; N'\,|\operatorname{Cov}_\mu(\mathbf{1}_A, f)|
  \;\le\; \frac{2\bar f N'}{m} \;=\; \frac{2N}{m},
\]
which is~\eqref{eq:sharp-centred}.

\smallskip\noindent\emph{The refined second-factor bound as a
corollary.} Substituting the bound $|p_{xy}(Q) - p_{xy}(Q-z)| \le 2/m$
into the closed-form identity noted above and using $\bar f \ge 1$,
\begin{equation}\label{eq:second-factor}
  \bigl|p_{xy}(Q) - \Pr(A \mid \bar B)\bigr|
  \;=\; \frac{m}{m - \bar f}\,\bigl|p_{xy}(Q) - p_{xy}(Q-z)\bigr|
  \;\le\; \frac{m}{m - \bar f}\cdot\frac{2}{m}
  \;\le\; \frac{2}{m - 1}.
\end{equation}
The first factor of~\eqref{eq:diff-final} satisfies
$1 - \bar f/m \le (m-1)/m$, and multiplying recovers
$|p_{xy}(Q) - p_{xy}(Q-z)| \le \frac{m-1}{m}\cdot\frac{2}{m-1}
= \frac{2}{m}$ consistently with~\eqref{eq:one-elem-perturb}.
\end{proof}

\begin{remark}[Structure of the FKG invocation]\label{rem:one-elem-perturb-fkg}
The proof above makes the FKG/Ahlswede--Daykin application to
Lemma~\ref{lem:one-elem-perturb} fully explicit. Specifically:
(i)~the distributive-lattice structure
$(\mathcal L(Q-z), \le_{\mathcal D})$ with reference extension $\tau$
chosen to separate $\operatorname{pred}(z)$ from $\operatorname{succ}(z)$
is constructed in Step~2;
(ii)~the monotonicity of $\mathbf{1}_A$, $P$, $S$ and of the product-space
indicators $\mathbf{1}_{B_\pm}$ is verified in Step~3 (items~(a)--(e));
(iii)~the Ahlswede--Daykin inequality
\eqref{eq:FKG-lattice} on the product lattice
$\mathcal X = \mathcal L(Q-z) \times \{1, \ldots, m\}$ is applied in
Step~4 to derive the sign constraints~\eqref{eq:AD-signs}, reducing
the scope dependency to a \emph{single} named quantitative lemma ---
the Kahn--Saks~\cite[Lemma~2.2]{KahnSaks1984} /
Brightwell~\cite[Theorem~4.1]{Brightwell1999} per-coordinate covariance
bound $|\operatorname{Cov}_\mu(\mathbf{1}_A, S)|,
|\operatorname{Cov}_\mu(\mathbf{1}_A, P)| \le \bar f/m$, which we
invoke as a named result (its proof is a combinatorial computation
against the $1$-Lipschitz bounds~\eqref{eq:fS-fP-lipschitz} and the
AD sign constraints, not an independent external input); and
(iv)~the final magnitude $|\Delta| \le 2/m$ is a direct
consequence of this pinned-down invocation via~\eqref{eq:delta-as-cov}
and~\eqref{eq:cov-as-mu}. The gap flagged in the pc-1f5e diagnosis of
eq:diff-final as a tautology is thereby resolved by an internal
derivation that localizes the FKG dependency to a specific cited
lemma, rather than by a generic "Brightwell-style'' invocation. See
the mg-391c audit.
\end{remark}
...
\begin{itemize}[leftmargin=*]
 ...
 \textbf{Open (gap).} See Section~\ref{sec:layered-reduction} and
 Lemma~\ref{lem:layered-reduction} below. The reduction is correct on
 the bulk (Case~(a)) but its window-perturbation estimate
 \eqref{eq:window-pert} (Case~(b)) is wrong --- it claims $o_K(1)$
 where the truth is $\Theta(1)$. See the \textbf{[GAP:]} note
 following the proof of Lemma~\ref{lem:layered-reduction}.

 \item[\textbf{G4}] \emph{Layered-width-3 balanced-pair lemma.}
 \textbf{Partially discharged; open for the deep irreducible case.}
 See Section~\ref{sec:g4-balanced-pair} and
 Lemma~\ref{lem:layered-balanced} below.
 Lemma~\ref{lem:layered-balanced} is established for depth $K = 1$,
 for layer-ordinal-reducible posets, and for irreducible posets of
 bounded size $|X| \le 3(3w+1)$. The \emph{deep irreducible} case is
 not covered: its proof (Case~C1) relies on
 Lemma~\ref{lem:window-localization}, which is invalid for $w \ge 1$.
 See the \textbf{[GAP:]} notes after
 Lemma~\ref{lem:window-localization} and after the proof of
 Lemma~\ref{lem:layered-balanced}. Together with the G3 gap, the deep
 irreducible regime of Proposition~\ref{prop:step7}(iii) is open.
...
\begin{remark}[Cross-reference to Hypothesis~\ref{hyp:arith}; Lean
formalisation note]\label{rem:G2-hyp-arith}
The constants below ($c_5(T), c_6, K(T), \delta_0(\gamma),
\varepsilon$) feed into the cascade closure
inequality~\eqref{eq:G2} of Proposition~\ref{prop:G2}; for the
inequality to hold at large $n$, the arithmetic-richness
Hypothesis~\ref{hyp:arith} of Section~\ref{sec:main-thm} must be in
force. The companion Lean formalisation takes Hypothesis~A as an
explicit hypothesis (\texttt{HypothesisArithmetic}) of the in-tree
headline \texttt{width3\_one\_third\_two\_thirds}, and carries all of
Steps~1--7 --- including the cascade closure inequality~\eqref{eq:G2}
--- as the disclosed axiom \texttt{stepsOneToSevenCascade}. The
constants of this section are paper-side bookkeeping for the
Steps~1--7 assembly. Note that the audit
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd) found a
load-bearing flaw \emph{upstream} of these constants --- the Step~2
per-fiber staircase error $\varepsilon$ is not actually delivered as
an $o(1)$ quantity (see the \textbf{[GAP:]} note after
Proposition~\ref{prop:per-fiber} in \texttt{step2.tex}) --- so the
constant $\varepsilon$ used below, and hence the inequality
\eqref{eq:G2}, currently rest on an open gap.
\end{remark}
...
\section{G3: Layered-decomposition reduction via size-minimality}
\label{sec:layered-reduction}

This section addresses GAP\,G3: the ``reduction'' branch of
...
decompositions. As recorded in the \textbf{[GAP:]} note after the
proof of Lemma~\ref{lem:layered-reduction}, this section's reduction
is incomplete: its window-perturbation estimate is incorrect, leaving
the deep regime open.

\begin{remark}[Paper-only: G3 is not on the in-tree Lean headline
path]\label{rem:g3-paper-only}
The companion Lean formalisation
(\texttt{lean/OneThird/Step8/LayeredReduction.lean}) formalises
this section's main lemma (Lemma~\ref{lem:layered-reduction})
end-to-end as \texttt{lem\_layered\_reduction}, together with its
underlying cut lemma \texttt{lem\_cut} and the supporting
\texttt{Step8/LayeredBalanced.lean}'s \texttt{windowLocalization}.
However, those Lean theorems are \emph{not consumed} by the in-tree
headline call chain
(\texttt{MainTheorem.lean}~$\to$~\texttt{MainAssembly.lean}~$\to$
\texttt{lem\_layered\_balanced}~$\to$ \texttt{Case3WitnessProof.lean}):
the deep-regime $K \ge K_0$ discharge is absorbed into the F3
strong induction of \texttt{Case3WitnessProof.lean} via
\texttt{LayeredDecomposition.compactify} descent, which strictly
decreases \texttt{Fintype.card} at every recursive call. The
depth-dichotomy framing $K \ge K_0$ vs.\ $K < K_0$ of the present
section is therefore a \emph{paper-side} organisation; the Lean
formalisation uses G4 (\texttt{lem\_layered\_balanced}) uniformly at
every depth. Lemma~\ref{lem:layered-reduction} below is preserved in
the paper as documentation of the originally intended mechanism and
is faithfully formalised in Lean; readers tracking the in-tree
dispatch should view it as supplementary rather than headline.
\end{remark}
...
Lemma~\ref{lem:layered-reduction} (below) — a balanced pair —
holds on $X \setminus X^{\mathrm{exc}}$, then
item~\ref{it:lift-perturb} transfers it to $P$ with a
pairwise-probability slack
\[
  \frac{2\,C_{\mathrm{exc}}(T)}{|X| - C_{\mathrm{exc}}(T) + 1}
  \;=\; o(1)
  \qquad \text{once $|X| \ge n_0(\gamma, T)$ of~\eqref{eq:n0-main},}
\]
which is absorbed by the perturbation slack of
Lemma~\ref{lem:layered-reduction} for $K_0$ chosen large enough
(see the threshold definition in
Lemma~\ref{lem:layered-reduction}).  Throughout the remainder of
this section we may therefore assume $P = X$ is exactly layered in
the sense of Definition~\ref{def:layered}.
\end{remark}
...
\begin{lemma}[Layered reduction; GAP\,G3 --- incomplete, see gap note]\label{lem:layered-reduction}
Fix $\gamma > 0$. There is an integer $K_0 = K_0(\gamma, w)$ such
that the following holds. Let $P$ be a \emph{size-minimal} width-$3$
$\gamma$-counterexample on ground set $X$ admitting a layered
decomposition of interaction width $w$ and depth $K \ge K_0$. Then
$P$ has a balanced pair, contradicting the assumption that $P$ is a
$\gamma$-counterexample.
\end{lemma}

\begin{remark}[Size-minimality framing]\label{rem:size-minimal-framing}
The lemma is stated as a one-shot reductio against \emph{size}-minimal
counterexamples: among all width-$3$ $\gamma$-counterexamples (for
the given $\gamma$), $P$ has the smallest ground set $|X|$. The
earlier formulation of this paper used $\gamma/2$-recursion,
packaging $A$ as a smaller $(\gamma/2)$-counterexample and deferring
the contradiction to the recursive call. Size-minimality contradicts
directly: any proper induced subposet $A \subsetneq P$ has
$|A| < |X|$, so by size-minimality $A$ is not a
$\gamma$-counterexample at the \emph{same} parameter $\gamma$, hence
$A$ has a balanced pair, which lifts to $P$ by Step~3 (bulk) or
Step~5 (window). The dependence of $K_0$ on $\gamma$ is one-sided
(driven by the window-perturbation bound alone), with no compounding
through iterated halving.
\end{remark}

\begin{proof}
Assume $K \ge K_0 := \max\bigl(2w+2,\; \lceil 2/\gamma \rceil + 6w +
4\bigr)$. We produce a balanced pair in $P$.
...
\smallskip\noindent\emph{Step 2: pick the heavy side.} Both $A$ and
$B$ are nonempty (since $w < k < K - w$ and each band $L_i$ contains
some element of $X$ in the size-stable regime $|X| \ge n_0(\gamma,
T)$). By symmetry under reversing $\le_P$, rename so that $|A| \ge
|B|$; in particular $A$ is a \emph{proper} induced subposet of $P$ on
ground set $A \subsetneq X$, hence $|A| < |X|$. This is the only
property of $A$ used by the size-minimality argument below.
...
\begin{equation}\label{eq:bulk-pxy}
  p_{xy}(P) \;=\; p_{xy}(P^\sharp) \;=\; p_{xy}(A)
  \qquad
  \text{for every $(x,y)$ incomparable in $P$ with $x,y \in A \setminus W$.}
\end{equation}

\smallskip\noindent\emph{Step 4: minimality discharges $A$.} Since
$P$ is size-minimal among $\gamma$-counterexamples and $|A| < |X|$,
$A$ is \emph{not} a $\gamma$-counterexample (at the same parameter
$\gamma$). Therefore $A$ has some incomparable pair $(x,y) \subseteq
A$ with $p_{xy}(A) \in [1/3, 2/3]$ — i.e., a balanced pair in $A$.
(If $A$ were a chain, the chain hypothesis would propagate into the
size-minimality counterexample fallback handled upstream by the
small-$|X|$ branch of Theorem~\ref{thm:main}; in the regime
$|X| \ge n_0(\gamma, T)$ used here, $A$ inherits the non-chain
structure of $P$ via the cut.)

\smallskip\noindent\emph{Step 5: lifting the balanced pair to $P$.}
Split on whether the balanced pair from Step~4 lies in the bulk
$A \setminus W$ or touches the window $W$.

\textbf{Case~(a) — bulk:} $(x,y) \subseteq A \setminus W$. By
\eqref{eq:bulk-pxy}, $p_{xy}(P) = p_{xy}(A) \in [1/3, 2/3]$, so
$(x,y)$ is a balanced pair in $P$ as well. Done.

\textbf{Case~(b) — every balanced pair of $A$ touches the window:}
take any balanced pair $(x, y)$ of $A$ with $\{x, y\} \cap W \ne
\emptyset$. By the same argument as Step~3 applied to $(x,y)
...
\begin{equation}\label{eq:window-pert}
  |p_{xy}(P) - p_{xy}(A)|
  \;\le\;
...
With our choice $K_0 \ge \lceil 2/\gamma \rceil + 6w + 4$, the
perturbation is at most $1/3 - \gamma/2$, in particular within the
slack budget that keeps $p_{xy}(P)$ inside $[1/3, 2/3]$ given
$p_{xy}(A) \in [1/3, 2/3]$. Hence $(x,y)$ is a balanced pair in $P$
as well. Done.

\smallskip\noindent\emph{Conclusion.} In either case $P$ has a
balanced pair, contradicting the assumption that $P$ is a
$\gamma$-counterexample.
\end{proof}

\GAP{The estimate \eqref{eq:window-pert} in Case~(b) is \emph{wrong}.
The inequality
$|p_{xy}(P) - p_{xy}(A)| \le |\mathcal L(P) \triangle \mathcal
L(P^\sharp)| / |\mathcal L(P)|$ is valid, but its right-hand side is
\emph{not} $o_K(1)$: it is $\Theta(1)$, bounded away from $0$
uniformly in $K$. The extensions in
$\mathcal L(P) \setminus \mathcal L(P^\sharp)$ are exactly those that
reverse some incomparable cross-pair inside the bounded window $W$;
the ratio $|\mathcal L(P^\sharp)| / |\mathcal L(P)|$ is the
probability that a uniform random extension respects the ordinal
order on $W$, a constant in $\big(0, 1\big)$ depending only on the
bounded window (it lies in $[1/(6w)!,\,1)$) and \emph{not} decaying
with $K$. The reasoning ``$W$ has bounded size while
$|\mathcal L(P)|$ grows'' conflates the bounded \emph{window} with a
bounded \emph{count}: the number of full extensions reversing a window
pair grows in proportion to $|\mathcal L(P)| \sim 2^K$, not with
$|W|$. Hence Case~(b) --- the case where every balanced pair of $A$
touches the cut window --- is not closed, and
Lemma~\ref{lem:layered-reduction} is an open gap for the deep regime.
Together with the gap in Case~C of Lemma~\ref{lem:layered-balanced},
this leaves the deep \emph{irreducible} layered width-$3$ case
unproven by either route. See
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).}
...
$K \ge K_0$ goes to Lemma~\ref{lem:layered-reduction}. The G4 branch
does not depend on size-minimality (it is a direct averaging
argument), so the size-minimality framing of
Lemma~\ref{lem:layered-reduction} only loads in the deep regime.
\end{remark}
...
This section addresses GAP\,G4: every layered width-$3$ poset that is
not a chain has a balanced pair, via ordinal-sum factorisation and a
direct averaging argument on a bounded interaction window. The
intended design pairs this with Lemma~\ref{lem:layered-reduction} (G3)
along a depth dichotomy --- shallow layerings ($K < K_0$) handled
here, deep layerings ($K \ge K_0$) by G3. As recorded in the
\textbf{[GAP:]} notes below (and in Section~\ref{sec:roadmap-residual}),
this section is \emph{not} complete: the deep \emph{irreducible} case
is established by neither route, and remains an open gap.
...
\begin{remark}[Lean formalisation route]\label{rem:layered-balanced-lean}
The paper proof of Lemma~\ref{lem:layered-balanced} below (proceeding
through ordinal-sum factorisation, window localization, and a direct
averaging argument on the bipartite reduct) is presented in the
``direct averaging on a bounded interaction window'' shape. The
companion Lean formalisation
(\texttt{lean/OneThird/Step8/LayeredBalanced.lean},
\texttt{lem\_layered\_balanced}) takes a different route at the
$K \ge 2$ branch: rather than calling
Lemma~\ref{lem:window-localization} and
Proposition~\ref{prop:in-situ-balanced} explicitly, it routes through
\texttt{OneThird/Step8/OptionC/Case3WitnessProof.lean},
\texttt{Case3Witness\_proof}, an F3 strong induction on
\texttt{Fintype.card} that case-splits on $K \in \{1, 2, \ge 3\}$ and on
reducibility at each layer. The small-$|P|$ base case is discharged
by the enumeration certificate of Remark~\ref{rem:residual-base}
below (\texttt{mg-4d7b}); the $K \ge 3$ irreducible leaves are
discharged by the F5a Bool certificate
(\texttt{BoundedIrreducibleBalancedInScope.lean},
\texttt{case3\_certificate}). Both routes share the same gap: the deep
irreducible case (see the \textbf{[GAP:]} note after the proof of
Lemma~\ref{lem:layered-balanced}). In the companion Lean development
this case is carried by the disclosed axiom
\texttt{lem\_layered\_balanced\_irreducible\_base}; in the paper it is
the open residual described there. The two routes therefore prove the
same theorem under the same hypotheses \emph{and with the same open
gap}; they differ only in dispatch shape.
\end{remark}
...
\GAP{The probability identity in Lemma~\ref{lem:window-localization}
is \emph{not valid as stated} for interaction width $w \ge 1$. The
proof claims $(X_-, X_0, X_+)$ with $X_0 = W(i,j)$ is an ordinal-sum
decomposition witness, i.e.\ that every $z \notin W(i,j)$ is comparable
in $P$ to \emph{every} element of $W(i,j)$. By~(L2), an element $z$ in
band $L_k$ is forced below an element of band $L_\ell$ only when
$k + w < \ell$. An element $z$ in a band $L_k$ with
$\min(i,j) - 2w \le k < \min(i,j) - w$ lies outside $W(i,j)$, yet is
\emph{not} (L2)-forced below the bottom boundary band
$L_{\min(i,j)-w}$ of $W(i,j)$: the cut below $W(i,j)$ is not clean. No
fixed padding repairs this --- a layer-ordinal-irreducible poset
(Definition~\ref{def:layer-reducible}) has \emph{no} clean cut
anywhere, so its only ordinal middle pieces are $\varnothing$ and $X$
itself. The lemma is correct only when the two window boundaries
$\min(i,j)-w$ and $\max(i,j)+w$ are themselves indices of
layer-ordinal reducibility (Definition~\ref{def:layer-reducible}); in
that restricted form it is subsumed by
Corollary~\ref{cor:reducibility-transfer}. The unrestricted statement
above, and the Case~C1 step of Lemma~\ref{lem:layered-balanced} that
consumes it, are an open gap; see the audit
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).}
...
function of $w$. A direct machine-checked enumeration verifies
that every such configuration contains a balanced pair: the
companion Lean formalisation closes the parameter cells
$(K, w) \in \{(3,1), (3,2), (3,3), (3,4), (4,1)\}$ via the F5a Bool
certificate \texttt{case3\_certificate}
(\texttt{lean/OneThird/Step8/Case3Enum/}, discharged by
\texttt{native\_decide}), and the residual cap-$1$ cells
$(K, w)$ with $K \in \{4, \ldots, 10\}$ and $w \in \{2, 3, 4\}$
(subject to $K \le 2w + 2$) are exhaustively verified by Python
enumeration; the certificate is recorded as
\texttt{enum\_cap5\_certificate.json}
(\texttt{lean/scripts/enum\_cap5.py}, \texttt{enum\_cap5\_K10.py};
companion work item \texttt{mg-4d7b}). The verification covers
roughly $1.72\times 10^6$ configurations across $15$ residual cells
and reports no counterexamples. We record the finiteness portion of
this statement as Lemma~\ref{lem:enum} below, and the explicit
verification as Remark~\ref{rem:enumeration}.
\end{proof}
...
\GAP{Case~C of the proof above is an open gap. It relies on
Lemma~\ref{lem:window-localization}, whose pair-identity is invalid for
$w \ge 1$ (see the \textbf{[GAP:]} note following that lemma). With the
corrected window localization, a proper clean window $W \subsetneq X$
exists only when $P$ is layer-ordinal reducible --- a case already
handled by Case~B --- so for a genuinely \emph{irreducible} $P$ the
only sub-case is C2, $W = X$, which requires $|X| \le 3(3w+1)$. That
size bound \emph{fails} for the irreducible layered width-$3$ posets
that genuinely exist: e.g.\ $\beta = \{1,\dots,K\}$ with
$i <_P j \Leftrightarrow j - i > 2$ is width-$3$, non-chain, layered
with $\mathrm{band} = \mathrm{id}$ and $w = 2$, layer-ordinal
\emph{irreducible} (every cut $k\mid k+1$ is straddled by the
incomparable pair $(k,k+1)$), and has $K$ \emph{unbounded}. So
Lemma~\ref{lem:layered-balanced} is established only for $K = 1$, for
layer-ordinal-reducible $P$ (Case~B), and for irreducible $P$ with
$|X| \le 3(3w+1)$ (Case~C2). The \emph{deep irreducible} case ---
which is exactly the case a minimal counterexample falls into, since a
minimal counterexample is indecomposable and large --- is not covered.
This is the genuine residual recorded as the Lean axiom
\texttt{lem\_layered\_balanced\_irreducible\_base}; the statement is
strongly supported numerically (mg-44f1: an exact-rational search over
$1\,118\,061$ posets found no counterexample) but is \emph{unproven}.
See \texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).}

\begin{remark}[Where the old argument broke, and what the induction
does and does not fix]\label{rem:old-vs-new}
...
any further windowing.

\emph{This remark over-claimed in earlier drafts and is corrected
here.} The strong induction fixes the \emph{structure} of the argument
(no $\gamma$-recursion, an honest well-founded descent) but it does
\emph{not} close the mathematical gap: Case~C1 still relies on
Lemma~\ref{lem:window-localization}, whose ordinal-decomposition claim
is invalid for $w \ge 1$. With the corrected window localization, a
proper clean window exists only in the reducible case (subsumed by
Case~B), and the irreducible Case~C2 is bounded-size only, leaving the
deep irreducible regime open --- see the \textbf{[GAP:]} notes after
Lemma~\ref{lem:window-localization} and after the proof of
Lemma~\ref{lem:layered-balanced}. The Lean formalisation scheme of
\S\ref{sec:g4-balanced-pair} (the ``well-founded recursion framework'',
work item F3, and the ``chained \texttt{hasBalancedPair\_lift} over an
\texttt{OrdinalDecomp} chain'', work item F4) faithfully formalises the
\emph{reducible} descent; the deep irreducible base case is carried in
Lean as the disclosed axiom
\texttt{lem\_layered\_balanced\_irreducible\_base}.
\end{remark}

\begin{remark}[Enumeration of configurations; mg-4d7b
certificate]\label{rem:enumeration}
The enumeration invoked in Lemma~\ref{lem:enum} is a finite but
$w$-parametrised check, organised in two parts in the companion
Lean formalisation:

\smallskip\emph{Tight case $w = 0$.} The Lean-side helper
\verb|lem_layered_balanced_subtype|
(\texttt{lean/OneThird/Step8/LayeredBalanced.lean}) shows that every
incomparable pair lies in a single band, each band is an antichain
of size $\le 3$, and Case~1 of
Proposition~\ref{prop:in-situ-balanced} applies directly to any
within-band pair: the swap involution gives $\Pr[x <_L y] = 1/2$.

\smallskip\emph{Bounded case $w \in \{1, 2, 3, 4\}$.} The
configurations with $|A| = 3$ whose two-sided profiles form a
$\preceq$-antichain are exhaustively enumerated by the Lean
\texttt{Case3Enum} framework
(\texttt{lean/OneThird/Step8/Case3Enum/}) modulo the symmetries of
(L1). The parameter cells $(K, w) \in \{(3,1), (3,2), (3,3),
(3,4), (4,1)\}$ are closed in tree by the F5a Bool certificate
\texttt{case3\_certificate}, discharged uniformly by
\texttt{native\_decide}. The residual cap-$1$ cells, with $K \in
\{4, \ldots, 10\}$ and $w \in \{2, 3, 4\}$ subject to $K \le 2w+2$
(a total of $15$ parameter cells), are verified by an out-of-tree
Python enumeration (\texttt{lean/scripts/enum\_cap5.py} for $K \le
9$ and \texttt{enum\_cap5\_K10.py} for the $K = 10$, $w = 4$ cell;
companion work item \texttt{mg-4d7b}). The enumeration covers
roughly $1.72\times 10^6$ width-3 non-chain configurations across
the $15$ cells, applying a $O(|Q|^2)$ symmetric-pair fast path
(discharging the bulk of configurations via the Case~1
swap-involution argument) with a full linear-extension subset-DP
fallback for the residue. The accompanying certificate
\texttt{enum\_cap5\_certificate.json} records the per-cell
configuration count and verifies that every enumerated configuration
admits a balanced pair; a counterexample would halt the enumeration
and be reported. No counterexample is found in any cell.

\smallskip Together, the two parts of the enumeration cover every
$(K, w)$ tuple operative under the cap-$1$ (singleton-band) +
cap-$5$ ($w \le 4$) profile of Lemma~\ref{lem:enum}'s irreducible
sub-class. In particular, the cardinality constraint
$|Q| \le K \le 2w + 2$ derived from cap~$1$ + cap~$2$ + cap~$5$
forces $|Q| \le 10$ on this cap profile, so the enumeration above
exhausts the bounded-depth case as long as $w_0(\gamma) \le 4$
(Remark~\ref{rem:w0-constant} of \texttt{step7.tex}).
\end{remark}

\begin{remark}[Interface with G3, and the open depth-gap]\label{rem:g3-g4-interface}
The intended design is a depth dichotomy: shallow layerings
($K < K_0(\gamma,w)$) handled by G4
(Lemma~\ref{lem:layered-balanced}), deep layerings
($K \ge K_0$) by G3 (Lemma~\ref{lem:layered-reduction}). As the
\textbf{[GAP:]} notes attached to those two lemmas record, neither
route is complete for a \emph{deep irreducible} layered width-$3$
poset: G4's Case~C1 relies on the invalid
Lemma~\ref{lem:window-localization}, and G3's Case~(b) relies on the
incorrect estimate \eqref{eq:window-pert}. A minimal
$\gamma$-counter\-example, being indecomposable
(Remark~\ref{rem:decomp-reduction}) and large, has precisely a deep
irreducible layering, so this is not a vacuous regime. The
established content is: G4 covers $K = 1$, layer-ordinal-reducible
posets, and irreducible posets of bounded size $|X| \le 3(3w+1)$; G3
covers deep posets whenever a heavy cut side carries a
\emph{bulk} balanced pair (Case~(a)). The remaining obligation --- the
deep irreducible case, equivalently the unbounded extension of
Proposition~\ref{prop:in-situ-balanced} Cases~2--3 --- is an open
mathematical gap, not a finite enumeration. The companion Lean
formalisation carries it as the disclosed axiom
\texttt{lem\_layered\_balanced\_irreducible\_base}; see
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).
\end{remark}

\begin{remark}[Residual Case-3 enumeration; mg-4d7b certificate]
\label{rem:residual-base}
The closing step of the $K \ge 2$ case of
Lemma~\ref{lem:layered-balanced} above invokes
Proposition~\ref{prop:bipartite-balanced} on an adjacent band pair
$M_i \cup M_{i+1}$ of the irreducible reduct $Q^\star$ and asserts that
the resulting balanced pair is balanced in $P$ ``by chained ordinal-sum
isolation''. This step is routine when $Q^\star$ itself has depth
$K^\star = 2$ (the bipartite restriction $M_i \cup M_{i+1}$ \emph{is}
all of $Q^\star$) and when the interaction width $w = 0$ (each band is
an ordinal-sum summand on its own, so restriction to a single band
preserves the $\Pr[x <_L y]$ marginal). The remaining regime --
$w \ge 1$ and $K^\star \ge 3$ -- is not a direct restriction argument
because elements in near bands $M_{i-w},\dots,M_{i-1}$ and
$M_{i+2},\dots,M_{i+1+w}$ may be comparable to $M_i \cup M_{i+1}$ in
a direction-variable way, so the marginal distribution on a
within-layer pair $(x,y) \subseteq M_i$ in $Q^\star$ can differ from
its marginal in $Q^\star|_{M_i \cup M_{i+1}}$.

For this residual regime the paper appealed to a bounded finite
enumeration, on the ground that window localization
(Lemma~\ref{lem:window-localization}) caps the size of $Q^\star$ at
$|Q^\star| \le 3(3w+1)$ and the depth at $K^\star \le 2w+2$.

\GAP{This boundedness claim is \emph{false}. Window localization is
invalid for $w \ge 1$ (see the \textbf{[GAP:]} note after
Lemma~\ref{lem:window-localization}), so it does not bound
$|Q^\star|$, and the irreducible residual is genuinely
\emph{unbounded}: the poset $\{1,\dots,K\}$ with
$i <_P j \Leftrightarrow j - i > 2$ is irreducible, width-$3$,
$w = 2$, with $K$ arbitrary. The finite enumeration described below
therefore covers only the \emph{bounded} cells $|Q| \le 3(3w+1)$
(in the cap-$1$ singleton-band sub-slice, $|Q| = K \le 2w+2 \le 10$);
it does \emph{not} discharge the unbounded irreducible regime. That
regime is the genuine open residual (Lean axiom
\texttt{lem\_layered\_balanced\_irreducible\_base}); see
\texttt{docs/onethird-math-finished-audit.md} (mg-5fdd).}

The enumeration carried out --- complete for the bounded cells only
--- is as follows. With $w \in \{1,2,3,4\}$, under the cap-$1$
singleton-band profile + cap-$5$ bandwidth bound $w \le 4$, the
bounded parameter domain is $|Q| = K \le 2w + 2 \le 10$, yielding
$15$ $(K, w)$ tuples covering roughly $1.72\times 10^6$
configurations across $K \in \{2, \ldots, 10\}$ and
$w \in \{0, \ldots, 4\}$ subject to $K \le 2w + 2$. The verification
proceeds in two layers: the parameter cells
$(K, w) \in \{(3,1), (3,2), (3,3), (3,4), (4,1)\}$ are closed in tree
by the F5a Bool certificate (Lean:
\texttt{lean/OneThird/Step8/Case3Enum/}, \texttt{case3\_certificate},
via \texttt{native\_decide}); the residual $15$ cells with
$K \in \{4, \ldots, 10\}$ and $w \in \{2, 3, 4\}$ are verified by
Python enumeration (\texttt{lean/scripts/enum\_cap5.py} for
$K \le 9$; \texttt{enum\_cap5\_K10.py} for $K = 10$, $w = 4$;
companion work item \texttt{mg-4d7b}). The accompanying certificate
\texttt{enum\_cap5\_certificate.json} records the per-cell
configuration count and reports no counterexample. This is strong
evidence for, but not a proof of, the unbounded residual.
\end{remark}
