...
\]
which bypasses any choice of $n_0(\gamma, T)$: either the
constants satisfy it (in which case the cascade closes at every
$n \ge 2$) or they do not (in which case the finite-enumeration
base case of Remark~\ref{rem:small-n} handles the $1/3$--$2/3$
property directly). No $n \to \infty$ limit is taken: the cascade
produces a contradiction at the single value of $n$ at hand,
provided the constant condition holds. Verification of
$\gamma^2 c_5 c_6 (\delta - K(T)\varepsilon) \ge 2$ is a final
constants audit, outside the scope of G1/G2.
\end{remark}
...
which together close the cascade at every $n$. G1 is therefore
\emph{fully discharged}.
...
\begin{theorem}[Width-$3$ 1/3--2/3, conditional on Steps 1--7]
\label{thm:main-step8}
Every finite width-$3$ poset that is not a total order has a balanced pair.
\end{theorem}

\noindent
(Conditional only on the sealed outputs of Steps 1--7; the Step~8
...
\end{equation}
If~\eqref{eq:arith-cond} fails, the small-$n$ base
case~(Remark~\ref{rem:small-n}) closes the 1/3--2/3 property
directly, bypassing the cascade; otherwise (a) holds at every
$n \ge 2$.
...
(standard element-deletion coupling: each deletion changes any
pairwise probability by at most $2/(n{-}i{+}1)$ where $i$ is the
current ground-set size, iterated $k$ times; see e.g.~the argument
in Lemma~\ref{lem:layered-reduction} below, specialized from the
window $W$ to the window $X^{\mathrm{exc}}$) gives: for every
$x,y \in X \setminus X^{\mathrm{exc}}$,
...
the structural argument of Steps~1--7, closing the cascade of
Theorem~\ref{thm:main} on every~$n$ (above $n_0$ by the BK-expansion
argument, below $n_0$ by Lemma~\ref{lem:small-n}).
\end{remark}
...
\begin{itemize}[leftmargin=*]
 ...
 \ref{rem:small-n}); the cascade of Theorem~\ref{thm:main} closes
 for every~$n$ without relying on any omitted spectral argument.
 ...
 Step~5/6 constants: either this holds (cascade closes at every
 $n \ge 2$) or it fails (small-$n$ base case
 Remark~\ref{rem:small-n} handles the $1/3$--$2/3$ property directly).
...
\begin{enumerate}[nosep, label=(\arabic*)]
 ...
 $n \ge 2$ satisfies the inequality; otherwise the inequality fails
 for all large $n$ and one falls back to the small-$n$ base case
 (Remark~\ref{rem:small-n}). The present $c_5(T) = c_T'(T)$ comes
...
\downarrow 0$, independently of $T, \delta$. For $n < n_0(\gamma,
T)$, the finite-enumeration base case (Remark~\ref{rem:small-n})
handles the 1/3--2/3 property directly.
\end{remark}
...
$\gamma^2 c_5(T) c_6 \delta_0 \ge 8$, which either holds (cascade
closes at every $n \ge 2$) or fails (small-$n$ base case
Remark~\ref{rem:small-n} handles 1/3--2/3 directly); (ii) the Step~2
...
depend only on $\gamma$ and $T$, so no external audit is required.
...
$c_5^\star$ produced by Proposition~\ref{prop:single-triple}: for any
particular $\gamma$, either this holds for some choice of $T$ (in
which case the cascade closes and the case~(R) contradiction fires
at every $n\ge2$), or it does not, in which case
Remark~\ref{rem:small-n} discharges the 1/3--2/3 property by finite
enumeration without invoking the cascade. In either branch, the main
theorem follows.
...
\begin{enumerate}[nosep, label=(L\arabic*)]
 \item each \emph{band} $L_i$ has $|L_i| \le 3$;
...
occur when $|i-j| \le w$. We call $w$ the \emph{interaction width} and
$K$ the \emph{depth} of the layering.
\end{definition}

\begin{remark}\label{rem:layered-from-step7}
The outputs of Step~5(C) and of Proposition~\ref{prop:step7}(ii) each
produce such a partition, possibly after discarding an $O_T(1)$-size
exceptional set $X^{\mathrm{exc}} \subseteq X$ and deleting a
$o(1)$-fraction of layers. Since the induced subposet on $X \setminus
X^{\mathrm{exc}}$ differs from $P$ in $O_T(1)$ elements, if the
conclusion of Lemma~\ref{lem:layered-reduction} below fails on
$X \setminus X^{\mathrm{exc}}$ it still fails on $P$ after adjusting
$\gamma$ by a further factor of $1 - o_T(1)$; we absorb this adjustment
into the $\gamma/2$ in Lemma~\ref{lem:layered-reduction}. Throughout
this section we may therefore assume $P = X$ is exactly layered in the
sense of Definition~\ref{def:layered}.
\end{remark}
...
The proof has two pieces: an ordinal-sum \emph{window localization}
that reduces the probability to a bounded sub-poset, and a direct
averaging argument on the reduced bipartite instance.
...
For the probability identity: every $z \in X \setminus W(i,j)$ lies in
a band $L_k$ with $|k - \min(i,j)| > w$ and $|k - \max(i,j)| > w$, so
by~(L2) $z$ is comparable (in $P$) to every element of $W(i,j)$, and
the direction depends only on whether $k < \min(i,j) - w$ or
$k > \max(i,j) + w$. Thus $X$ splits as $X = X^- \sqcup W(i,j) \sqcup
X^+$ with $X^- <_P W(i,j) <_P X^+$ (element-wise). Every linear
extension of $P$ decomposes uniquely as a concatenation of linear
extensions of $X^-$, $P|_{W(i,j)}$, $X^+$ chosen independently; hence
the marginal distribution of $(x,y)$-order in a uniform extension of
$P$ equals the corresponding marginal in a uniform extension of
$P|_{W(i,j)}$.
...
layered sub-poset $Q = P|_{W}$. We now reduce further to the
genuinely bipartite case ($K = 2$) by iterated ordinal decomposition,
then prove the bipartite statement directly.

\begin{definition}[Layer-ordinal-sum reducible]
A layered poset $Q = M_1 \sqcup \cdots \sqcup M_r$ is \emph{layer-ordinal
reducible} at index $k$ if every pair $(u,v)$ with $u \in M_i$,
$v \in M_j$, $i \le k < j$, satisfies $u <_Q v$; equivalently,
$Q = (M_1\cup\cdots\cup M_k) \oplus (M_{k+1}\cup\cdots\cup M_r)$ as
posets.
\end{definition}

If $Q$ is reducible at $k$, then $\mathcal L(Q) = \mathcal L(Q_1)
\times \mathcal L(Q_2)$ (Cartesian, concatenation), so any balanced
pair of $Q_1$ or $Q_2$ is a balanced pair of $Q$. Therefore it suffices
to exhibit a balanced pair in some \emph{irreducible} factor. An
irreducible layered piece with depth $\ge 2$ has some adjacent
band-pair $(M_i, M_{i+1})$ with an incomparable cross-pair.
...
If $K = 1$, then $P = L_1$ is a single antichain with $|L_1| \in
\{2, 3\}$ (since $P$ is not a chain and $|L_1| \le 3$). Every pair
$(x,y) \in L_1 \times L_1$ with $x \ne y$ is incomparable; by uniform
symmetry of linear orderings of an antichain, $\Pr[x <_L y] = 1/2$.

Assume $K \ge 2$. Since $P$ is not a chain, there is an incomparable
pair $(x, y) \subseteq X$, with $x \in L_i$, $y \in L_j$ and
$|i-j| \le w$ by Lemma~\ref{lem:window-localization}. Let
$Q := P|_{W(i,j)}$, a layered sub-poset of size $\le 3(3w+1)$ and
depth at most $3w + 1$. By Lemma~\ref{lem:window-localization},
\[
  \Pr_{L \sim \mathcal L(P)}[x <_L y]
  \;=\; \Pr_{L' \sim \mathcal L(Q)}[x <_{L'} y].
\]

Now iteratively decompose $Q$ by layer-ordinal-sum: while $Q$ has a
reducibility index $k$ (Def.\ above), replace $Q$ by whichever factor
$Q_1$ or $Q_2$ contains an incomparable pair (at least one does,
since an ordinal sum preserves chain-ness only if both factors are
chains, and $Q$ itself is not a chain). Each step strictly decreases
the number of bands. The iteration terminates at an \emph{irreducible}
layered sub-poset $Q^\star$ of depth $K^\star \ge 1$.

If $K^\star = 1$, we are in the single-antichain case already handled.

If $K^\star \ge 2$, irreducibility gives adjacent bands
$(M_i, M_{i+1}) \subseteq Q^\star$ with at least one incomparable
cross-pair. Apply Lemma~\ref{lem:window-localization} once more inside
$Q^\star$ to isolate this pair to the bipartite sub-poset
$M_i \cup M_{i+1}$: this sub-poset is height-$2$ with antichains of
size $\le 3$, so Proposition~\ref{prop:bipartite-balanced} produces a
balanced pair in $Q^\star|_{M_i \cup M_{i+1}}$, which by chained
ordinal-sum isolation is a balanced pair of $P$.
\end{proof}
...
In the deep regime ($K \ge K_0(\gamma, w)$), however,
Lemma~\ref{lem:layered-reduction} additionally produces a
\emph{proper sub-counterexample}, which is the stronger outcome
invoked when Theorem~\ref{thm:main} uses the minimality of $P$.
For $K < K_0$, Lemma~\ref{lem:layered-balanced} directly exhibits a
balanced pair, contradicting the counterexample hypothesis on $P$.
Together, Lemma~\ref{lem:layered-reduction} (G3) and
Lemma~\ref{lem:layered-balanced} (G4) exhaust
Proposition~\ref{prop:step7}(iii) with no gap.
\end{remark}
