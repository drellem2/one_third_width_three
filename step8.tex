...
$(M_i, M_{i+1})$ with an incomparable cross-pair; the remainder of
this subsection handles the genuinely bipartite case ($K = 2$)
directly.
...
Now iteratively decompose $Q$ by layer-ordinal-sum: while $Q$ is
layer-ordinal-reducible at some index $k$ in the sense of
Definition~\ref{def:layer-reducible}, apply
Corollary~\ref{cor:reducibility-transfer} and replace $Q$ by whichever
factor $Q_1$ or $Q_2$ contains an incomparable pair (at least one
does, since an ordinal sum preserves chain-ness only if both factors
are chains, and $Q$ itself is not a chain). Each step strictly
decreases the number of bands. By
Remark~\ref{rem:iterated-reduction}, the marginal $\Pr[x <_L y]$ is
preserved at every step. The iteration terminates at an
\emph{irreducible} layered sub-poset $Q^\star$ of depth
$K^\star \ge 1$.
...
balanced pair in $Q^\star|_{M_i \cup M_{i+1}}$, which by the chained
marginal invariance (window localization inside $P$ followed by
Corollary~\ref{cor:reducibility-transfer} at each reduction step) is
a balanced pair of $P$.
\end{proof}
