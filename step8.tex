...
\begin{remark}[Residual Case-3 enumeration; mg-307c]
...
$K^\star \le 2w+2$. With $w \in \{1,2,3,4\}$ from
Lemma~\ref{lem:bandwidth} of Step 7, the residual parameter range is
finite and admits direct machine-verified enumeration. The script
\texttt{lean/scripts/enum\_case3.py} enumerates every
layer-ordinal-sum irreducible layered width-3 poset $Q$ satisfying
(L1)--(L4) and containing an adjacent-band incomparable cross-pair
across the residual parameter range, and for each verifies that
some incomparable pair $(x,y) \in Q$ has
$\Pr_{L \sim \mathcal L(Q)}[x <_L y] \in [1/3, 2/3]$. The verification
succeeds uniformly: a fast $O(|Q|^2)$ symmetric-pair check discharges
the overwhelming majority of configurations via the swap-involution
argument of prop:bipartite-balanced Case 1, with a full
linear-extension enumeration as fallback for the asymmetric remainder.
The accompanying certificate \texttt{enum\_case3\_certificate.json}
records the parameter range covered and the number of configurations
verified. No counterexample is found; a counterexample would halt the
enumeration and be reported. See
\texttt{lean/scripts/README.md} for the scope covered and
instructions to extend to larger (w, K) regimes (same algorithm,
