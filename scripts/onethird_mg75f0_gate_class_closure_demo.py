...
mistake mg-60d3 made one level down.  So it also runs FIVE mutations that NEITHER
mg-60d3 NOR mg-5ad1 used, each a one-line edit landing in a field that was
uncompared until mg-75f0:
  ...
  M7  _transport_label_cheeger's volume      min -> max   ->  phi_t_cheeger
      normalisation
  M9  _width's antichain search direction    n..1 -> 1..n ->  width
...
probe rather than the BK side, M7 is a normalisation rather than a selector, and
M9 is neither -- it is a search-direction flip in a combinatorial helper that
touches no spectral quantity at all.  So between them they are not one shape.

AND ONE MUTATION THIS AUTHOR DID NOT CHOOSE, which is stronger than all four.

  M8  bk_pair_cut's Bernoulli variance      p(1-p) -> p   ->  frozen_ratio, ...

M5/M6/M7/M9 share a weakness that has to be said out loud: the person who wrote
the widening also picked them, and an author picks around the cases their own fix
...
WHAT "INVISIBLE BEFORE" MEANS, MEASURED RATHER THAN ASSERTED.  A left-column
exit 0 is two different findings and they are not equally strong, so this file
distinguishes them instead of averaging them.  For every case it digests the
pre-widening gate's ENTIRE stdout and compares it against the unmutated
pre-widening run:

  stdout DIFFERS, exit 0  ->  the pre-widening gate COMPUTED a quantity the
                              mutation moved, printed the moved value, and
                              passed anyway.  This is the class exactly.
  stdout IDENTICAL, exit 0 ->  the pre-widening gate never reached the mutated
                              code at all.  Still a blind spot -- the quantity
                              is committed, the document asserts it, and nothing
                              checked it -- but it is blindness by omission
                              rather than by absorption, and the widening's
                              achievement there is that it now EXERCISES the
                              code, not merely that it compares more fields.

Both columns are reported per row.  The distinction is the reason the widening
had to call mg-8b64's own row builder rather than compare more of what the gate
already happened to compute.
...
the dataset the gate reads, applies ONE source-level edit there with an
...
WHAT IS ASSERTED (this file is itself a control and exits non-zero if it fails):
      ...
      M9  width search direction exit 0 <- UNSEEN     exit 1
...
MUTATIONS = {
    ...
    "M8": {
        # The strongest row in this table, and it is not mine.  M5/M6/M7/M9 were
        # chosen by the same author who wrote the widening, which bounds how much
        # they can say -- an author avoids the mutations their own fix misses,
        # without meaning to.  M8 was authored by mg-7db4, for its own probe
        # battery (row N1), independently and before it had seen the widened
        # gate.  So it is the one mutation here that nobody who built the thing
        # under test selected.
    ...
    "M9": {
        # Deliberately the odd one out.  Every other row here moves a spectral
        # or transport quantity through a selector or a normalisation; this one
        # moves a PURELY COMBINATORIAL field by reversing a search direction, so
        # the table is not a single shape measured seven times.  `_width` scans
        # r = n..1 and returns the first r admitting an antichain, i.e. the
        # WIDTH; scanning 1..n returns 1 on every poset.  `width` is one of the
        # eighteen fields that was uncompared, and width is the parameter the
        # whole width-3 programme is indexed by.
        "desc": "_width's antichain search direction range(n,0,-1) -> "
                "range(1,n+1): the max-antichain scan returns the FIRST r that "
                "admits an antichain instead of the LAST, so width collapses to "
                "1 on every poset.  Not a selector and not a normalisation -- a "
                "search direction, in a purely combinatorial field",
        "seen_by": "UNSEEN -- used by neither mg-60d3 nor mg-5ad1",
        "fields": ["width"],
        "file": "scripts/onethird_mg8b64_L1b_bk_transport_transfer_probe.py",
...
def build_tree(root, mutation, gate_variant, pre_src):
    """An isolated tree with scripts/ + the dataset the gate reads, one
...
def run_case(mutation, gate_variant, pre_src, keep=False):
    ...
    try:
        ...
        return {"mutation": mutation, "gate": gate_variant,
                ...
                # The whole printed output, digested.  See the module docstring:
                # this is what separates "the gate computed the moved quantity,
                # printed it and passed" from "the gate never ran that code".
...
def main():
    ...
    widened_is_different = (pre_src != open(os.path.join(REPO, GATE)).read())
    print("=" * 78)
    ...
    # ---- was the mutation EXERCISED before, or merely uncompared? ----------
    # See the module docstring.  A left-column exit 0 whose stdout DIFFERS from
    # the unmutated left-column run is the class exactly: the gate computed the
    # moved quantity, printed it, and passed.  A left-column exit 0 whose stdout
    # is IDENTICAL means the pre-widening gate never reached the mutated code, so
    # the widening's achievement there is that it now exercises it.
    base_pre = next((r for r in results if r["mutation"] == "none"
                     and r["gate"] == "pre-widening"), None)
    for r in results:
        if r["gate"] != "pre-widening" or base_pre is None:
            continue
        if r["mutation"] == "none":
            continue
        r["pre_widening_stdout_differs"] = (
            r["stdout_sha256"] != base_pre["stdout_sha256"])
        r["pre_widening_verdict"] = (
            "EXERCISED AND ABSORBED -- the pre-widening gate computed a moved "
            "quantity, printed it, and exited 0"
            if r["pre_widening_stdout_differs"] else
            "NEVER EXERCISED -- the pre-widening gate did not reach the mutated "
            "code, so not one printed byte moved")
    ...
    if any(r["gate"] == "pre-widening" for r in results) and base_pre:
        print()
        print("HOW THE PRE-WIDENING GATE MISSED EACH ONE (measured, not asserted)")
        print(f"{'mutation':<10} {'stdout moved':<14} verdict")
        for r in results:
            if r["gate"] != "pre-widening" or r["mutation"] == "none":
                continue
            print(f"{r['mutation']:<10} "
                  f"{str(r['pre_widening_stdout_differs']):<14} "
    ...
    absorbed = [r["mutation"] for r in results
                if r["gate"] == "pre-widening"
                and r.get("pre_widening_stdout_differs")]
    ...
    report = {
        ...
        "route": "isolated tree per case (copy of scripts/ + the mg-8b64 "
                 ...
                 "gate is the real source at a PINNED revision, not a "
                 "reconstruction",
        "how_the_pre_widening_gate_missed_them": {
            "method": "sha256 of the pre-widening gate's entire stdout, "
                      "compared against the unmutated pre-widening run",
            "exercised_and_absorbed": sorted(absorbed),
            "never_exercised": sorted(
                r["mutation"] for r in results
                if r["gate"] == "pre-widening" and r["mutation"] != "none"
                and r.get("pre_widening_stdout_differs") is False),
        },
