...
\GAP{Remark~\ref{rem:G3-vacuity} concedes that
Lemma~\ref{lem:rect-stable-area} is \emph{vacuous} --- ``in the regime
the conclusion is non-trivial, no $A$ satisfies the hypotheses'' ---
and re-routes the quantitative work to Lemma~\ref{lem:many-nonconst}
at $\min(m,n)$-scale. This re-routing is an open gap: the
$|R|$-scale incompatibility bound is what
Theorem~\ref{thm:step4} requires, and a $\min(m,n)$-scale bound,
summed over incoherent blocks of total mass $|\Omega^\circ|$, can be
as small as $\Theta(|\Omega^\circ|/t)$ (one long thin block) --- a
factor $t$ short. The Case~B branch of
Proposition~\ref{prop:G5} accordingly delivers only an
$\varepsilon'\delta$-proportional boundary lower bound, not the
$c(\delta-C\varepsilon)$ asserted in Theorem~\ref{thm:step4}; since
$\varepsilon'=o(1)$, the constant $c_G$ of Theorem~\ref{thm:step4}
secretly contains a vanishing factor. The two-interface incompatibility
expansion engine is therefore not established at the mass scale the
dichotomy of Step~6 consumes. This is an open gap; see
