...
> **The "≈10 min" figure re-measured, and the apparent contradiction resolved (mg-8ff1).** Audit mg-7d24 could not reconcile "≈10 min" with an observed long-running process. Re-measured on this machine: `--nmax 10 --widths 4` completes in **430 s** and reports **1 717 902** classes — both **exactly** as documented above and in §4.2, so the figure is correct **for the runs this ticket actually performed**, which stop at `n = 10`. The discrepancy was **not** in this document but in the counting script's own docstring, which advertised `--nmax 11 --budget 600`. That is a different run: `n = 11` is a ≈13.9× level on top of `n = 10`, so it takes **≈1 h**, and — because `--budget` was checked only *between* levels — the 600 s budget could not interrupt it. Both halves are fixed: the docstring now carries measured per-level timings and no longer advertises `n = 11` as a short run, and `--budget` now actually bounds a level (§4.2). **Claim and measured behaviour now agree.**
