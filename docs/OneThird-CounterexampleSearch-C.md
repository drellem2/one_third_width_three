...
*Deliverable script: [`scripts/onethird_mg0eac_primitive_delta_search.py`](../scripts/onethird_mg0eac_primitive_delta_search.py). Certificates: [`data/onethird-mg0eac-primitive-delta-search.json`](../data/onethird-mg0eac-primitive-delta-search.json), [`data/onethird-mg0eac-ladder-profile.json`](../data/onethird-mg0eac-ladder-profile.json).*
...
| **widths ≥ 3, `n ≥ 12`** | — | **NOT COVERED** | **genuine gap** |

**The honest gap.** This search does not speak to **width ≥ 3 at `n ≥ 12`** at all. Two reasons not to weight it heavily, both **heuristics, not arguments**: (a) every known `β`-extremal family — Peczarski's ladders, Chen's `P(5k,5k)`, Sah's `Tₙ` — is **width 2**; (b) `δ` trends upward with width (the antichain, maximally wide, has the largest possible `δ = 1/2`), and Olson–Sagan report the smallest known `δ` at width `> 2` is `14/39 ≈ 0.3590`, far above `β`, with computer search to 9 elements finding nothing smaller. But a genuine low-`δ` wide poset at `n ≥ 12` would be missed here.
...
1. **Width ≥ 3 at `n ≥ 12`** — the one uncovered region (§6), and the only place a counterexample could still hide from this search.
...
Pure standard library. The script **asserts** every positive control (internal and the external Peczarski table) and **raises** `SubBetaHalt` on any `δ ≤ 1/3` candidate, so neither a silent engine regression nor an unreviewed counterexample claim can pass through.
...
* M. Olson, B. Sagan, *On the 1/3–2/3 Conjecture*, **Order 35, 581–596 (2018)**, arXiv:1706.04985. §6: their posets `A, B, C` with `δ = 6/17, 60/171, 37/106` coincide with Peczarski's `L₉;₁,₂,₃,₄`, `L₁₁;₁,₆`, `L₁₀;₁,₅`; smallest known `δ` at width `> 2` is `14/39`.
